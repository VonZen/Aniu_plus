...
class AniuService:
    ...
    def _run_body(
        ...
        *,
        ...
        try:
            ...
            llm_settings_snapshot = dict(settings_snapshot)
            if str(llm_settings_snapshot.get("run_type") or "").strip() == "trade":
                trade_settings = SimpleNamespace(**llm_settings_snapshot)
                llm_settings_snapshot["task_prompt"] = self._build_trade_execution_prompt(
                    trade_settings
                )
            try:
                (
                    ...
                    settings_snapshot=llm_settings_snapshot,
                    trigger_source=trigger_source,
                    schedule_id=schedule_id,
            ...
            if settings_snapshot.get("tg_notify_failure_enabled"):
                ...
                try:
                    send_telegram_failure_notification(
                        ...
                        detail_url=self._build_run_detail_url(settings_snapshot, run_id),
                    )
                except Exception:
                    logger.warning(
    ...
    def _extract_executed_actions(self, tool_calls: Any) -> list[TradeExecutionIntent]:
        return intents_from_proposals(
            self._evaluate_trade_proposals(
                self._extract_trade_proposals(tool_calls),
                run_type="trade",
                trade_enabled=True,
